...
export default async function ArticuloPage({ params }: Props) {
  ...
  return (
    <>
      <section className="bg-gradient-to-br from-[#0c4a6e] to-[#082f49] text-white py-14">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
        <div
          className="prose prose-lg prose-slate max-w-none
            prose-headings:text-[#0c4a6e] prose-headings:font-bold
        ...
        <div className="mt-12 bg-[#0c4a6e] text-white rounded-2xl p-6 text-center">
      ...
      {otros.length > 0 && (
        <section className="bg-[#f0f9ff] py-12">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <h2 className="text-xl font-bold text-[#0c4a6e] mb-6">Más artículos</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {otros.map((a) => (
                <Link
                  key={a.slug}
                  href={`/blog/${a.slug}`}
                  className="group bg-white rounded-2xl p-5 border border-sky-100 hover:border-[#0ea5e9] hover:shadow-md transition-all"
                >
                  <span className="inline-block bg-sky-50 text-[#0c4a6e] text-xs font-semibold px-3 py-1 rounded-full mb-3">
                    {a.categoria}
                  </span>
                  <h3 className="font-bold text-[#0c4a6e] text-base group-hover:text-[#0ea5e9] transition-colors leading-snug">
                    {a.titulo}
                  </h3>

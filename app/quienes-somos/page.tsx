  ...
  return (
    <>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-[#5b80d4] font-semibold text-sm uppercase tracking-widest mb-3">Nuestra historia</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={s.label} className="bg-[#f0f9ff] rounded-2xl p-6 text-center border border-sky-100">
                  <p className="text-3xl font-black text-[#2e3f8a]">{s.num}</p>
      ...
      <section className="py-16 bg-[#f0f9ff]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <p className="text-[#5b80d4] font-semibold text-sm uppercase tracking-widest mb-3">El equipo</p>
            <h2 className="text-3xl font-bold text-[#2e3f8a]">Quiénes le atienden</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {equipo.map((e) => (
              <div key={e.nombre} className="bg-white rounded-2xl p-7 border border-sky-100 shadow-sm">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#5b80d4] to-[#2e3f8a] flex items-center justify-center text-white font-bold text-xl mb-4">
                  {e.nombre.split(" ")[1][0]}
                </div>
                <h3 className="font-bold text-[#2e3f8a] text-xl">{e.nombre}</h3>
                <p className="text-[#5b80d4] text-sm font-semibold mb-3">{e.titulo}</p>
                  ...
                  <ul className="space-y-1">
                    {e.especialidades.map((esp) => (
                      <li key={esp} className="flex items-center gap-2 text-sm text-slate-600">
                        <span className="text-[#5b80d4] shrink-0">·</span>
                        {esp}
                      </li>
                    ))}
                  </ul>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <p className="text-[#5b80d4] font-semibold text-sm uppercase tracking-widest mb-3">Lo que nos diferencia</p>
            <h2 className="text-3xl font-bold text-[#2e3f8a]">Nuestra forma de trabajar</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {valores.map((v) => (
              <div key={v.titulo} className="flex gap-4 p-6 bg-[#f0f9ff] rounded-2xl border border-sky-100">
                <div className="w-10 h-10 rounded-xl bg-[#2e3f8a] flex items-center justify-center shrink-0 mt-0.5">

  ...
  if (resultado) {
    return (
      <div className="bg-[#1e3a8a] text-white rounded-2xl p-8 text-center">
        <div className="w-12 h-12 bg-[#4a80e4] rounded-full flex items-center justify-center mx-auto mb-4">
        ...
        {pasos.map((_, i) => (
          <div
            ...
            className={`h-1.5 flex-1 rounded-full transition-colors ${
              i <= paso ? "bg-[#4a80e4]" : "bg-slate-100"
            }`}
          />
        ))}
      </div>
      <p className="text-xs font-medium text-[#4a80e4] uppercase tracking-wide mb-3">
        Pregunta {paso + 1} de {pasos.length}
      </p>
      <h3 className="text-xl font-semibold text-[#1e3a8a] mb-6">{pasoActual.pregunta}</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {pasoActual.opciones.map((op) => (
          <button
            key={op.valor}
            onClick={() => elegir(op.valor)}
            className="text-left px-4 py-4 rounded-xl border-2 border-sky-100 text-slate-700 font-medium text-sm hover:border-[#4a80e4] hover:bg-sky-50 hover:text-[#1e3a8a] transition-all active:scale-98"
          >
            {op.texto}
          </button>
        ))}
